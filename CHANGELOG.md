...
- **Added a native four-channel Renaud DWBA structure factor for a bulk unit
  cell and its semi-infinite repetition.** ``SXRDCrystal.prepare_DWBA`` freezes
  the incident/reciprocal-exit electric fields and channel geometry for reuse,
  while ``F_DWBA_prepared`` reevaluates current bulk atomic and coherent-domain
  parameters without another optical solve. Complex channel-specific
  Waasmaier--Kirfel and anisotropic Debye--Waller factors are evaluated in the
  C++ extension. Physical bulk absorption comes from the complex DWBA
  wavevectors, so the optional empirical ``bulk_attenuation`` now defaults to
  zero. When requested for comparisons with a kinematical model, its envelope
  is applied continuously both within the top unit cell and between repeats,
  avoiding artificial anti-Bragg dips. ``CTRutil.set_atten_from_dwba`` can
  populate the scalar kinematical attenuation from the exact incident bulk
  wavevector, while the vectorized ``attenuation_from_dwba`` also includes an
  optional exit path for diagnostics. The optical API now exposes immutable
  ``LayeredElectricField`` objects with Renaud-signed ``kz``, ``A_plus``, and
  ``A_minus``; field samples are requested separately. Core wavefield and XRR
  glancing angles now use radians rather than degrees.

  ``SXRDCrystal.prepare_DWBA_Zmode`` is the convenience counterpart that does
  not require explicit glancing angles: it derives them from the Vlieg z-mode
  angle constraint of ``HKLVlieg.VliegAngles.anglesZmode`` (``fixed="in"``,
  ``"out"``, or ``"eq"``) and populates the diffractometer from the crystal
  itself — the bulk cell is the lattice, its energy sets the wavelength, and
  the orientation matrix is ``U = 1``, so the surface normal is the omega
  axis. Only ``chi = phi = 0`` is accepted, because a tilted inner sample
  circle would make ``alpha`` and ``gamma`` differ from the glancing angles of
  the layered optics. ``return_angles=True`` also yields the six z-mode angles
  in radians. ``examples/CTR/pt100_dwba_ctr_example.ipynb`` uses it for the
  specular ``(0 0 L)`` and non-specular ``(2 0 L)`` rods of Pt(100).

  ``SXRDCrystal.prepare_DWBA_from_angles`` is the inverse convenience API, for
  scans whose natural variable is the glancing angle rather than ``l`` — near
  the critical angle a rod is sub-critical only over a thin ``l`` interval, so
  ``l`` cannot resolve it. The in-plane indices and both glancing angles are
  given, and ``l`` follows from the Ewald condition
  ``Qz = k0 (sin(alpha_i) + sin(alpha_f))``; pass ``return_l=True`` to get it
  back. That conversion is also public on its own as
  ``SXRDCrystal.l_from_glancing_angles``, so callers can hand the result to
  ``prepare_DWBA`` and let its Ewald check verify the round trip. It is
  rejected when the reference out-of-plane axis has no surface-normal
  component, because ``Qz`` is then independent of ``l``.
...
- **Fixed ROI integration multiplying every corrected intensity by the size of
  ...
  ROI area, ``Corr_croi * roi_size / Corr_cpixel``, rather than the mean
  correction over the ROI, ``Corr_croi / Corr_cpixel``. The integrated
  intensity already carries the ``roi_size / cpixel`` rescaling from the
  background subtraction, so the area entered twice and intensities came out
  scaled by their ROI area -- a factor of order 200 to 500 on measured data.
  Worse, the projected ROI size varies over the detector, so two measurements
  of one crystal truncation rod recorded on opposite sides of the direct beam
  were scaled apart: on a measured rod the two branches differed by a factor
  1.96 with corrections enabled against 1.37 with them disabled, matching the
  1.43 ratio of their ROI sizes. This affected both stationary and
  rocking-scan integration, and appeared only when the solid-angle or
  polarization correction was switched on, since those are what enable the
  correction path at all. **Any integration run with a solid-angle or
  polarization correction has to be repeated.**
- Known, unfixed: the background ROI is corrected with the *center* ROI's mean
  correction factor rather than its own. Both are accumulated, and the
  background one is even stored as ``Cfactors_bgroi``, but only the center
  factor is applied. Since the background ROIs sit next to the signal ROI and
  the detector corrections vary smoothly, the two factors differed by at most
  0.04% on measured data, shifting the background-subtracted intensity by a
  median of 0.004%. Correcting it properly means moving the correction inside
  the background subtraction across all four background branches, including
  the error propagation, which is not worth the risk for that size of effect.

- **Fixed the polarization correction, which was evaluated in the wrong
  direction and with the wrong parameterization.** Both of pyFAI's arguments
  were passed in orGUI's own conventions rather than pyFAI's, and neither
  translation was made:
  - pyFAI places the polarization node at its own detector azimuth
    ``chi = -axis_offset``, while orGUI measures azimuth in the diffractometer
    frame, ``azimuth = chi + azimuthal_reference``. orGUI passed
    ``polarization_axis`` alone, leaving the azimuthal reference out. For the
    usual vertical-scattering setup that reference is 90 degrees, which
    rotates the polarization plane by a right angle and swaps the polarized
    and unpolarized directions of the detector.
  - ``polarization_factor`` is the *fraction* of horizontally polarized light,
    as in the ANA/ROD manual and in ``HKLVlieg.UBCalculator.polarization``,
    while pyFAI's ``factor`` is ``(Ih - Iv) / (Ih + Iv)``. The two are related
    by ``factor = 2 * fraction - 1`` and coincide only for a fully
    horizontally polarized beam, which is why this stayed hidden.
  Both corrections now go through ``Detector2D_SXRD.polarizationArray()``,
  which reproduces orGUI's own z-axis expression -- the ANA/ROD Appendix A
  formula -- to the precision pyFAI stores its result in. Measured on a real
  20 keV geometry with a 90 degree azimuthal reference, the previous call was
  wrong by -31% to +75% depending on the detector position, and along one
  crystal truncation rod it distorted the intensity by up to 34%, growing
  smoothly with L. Because the error varies across the detector, two
  measurements of the same rod recorded on different parts of it were
  corrected differently and did not agree. **Any integration or
...
- Stationary-scan integration can divide each image by its exposure time and
  ...
  scan integrated and reconstructed is normalized identically. A backend
...
- The footprint correction of rocking-scan integration can now use a beam
  ...
  scale factor absorbs it. The ``options`` dialog next to the ``footprint``
  checkbox now offers a measured profile read from a two-column text file,
  integrated numerically over the projected sample footprint. A sample height
  scan can be loaded directly and is differentiated into the profile
...
- The footprint corrections also accept analytical beam shapes other than a
  ...
  describe it; the dialog now offers a top hat, a trapezoid, a top hat with
  error-function flanks, a generalized normal and a skew normal alongside the
  Gaussian, each scaled to the entered full width at half maximum. The top hat
  is the slit-limited beam whose correction is linear in the incidence angle,
  and the trapezoid the beam two unmatched slits define, both of which
  previously had no representation at all. Arbitrary distributions are
  available in scripts as ``DistributionBeamProfile``, which takes any frozen
  ``scipy.stats`` distribution; a distribution whose density diverges, or
  whose center of mass does not converge, is rejected with an explanation
  rather than silently producing a cut-off-dependent correction.
- The integration correction switches moved from four checkboxes on the ROI
  integration tab into one non-modal ``Corrections and normalization ...``
  dialog, which stays open while scans are integrated. Under the button a
  status line abbreviates the enabled corrections in colour -- ``MASK``,
  ``SOLA``, ``POL``, ``LOR``, ``FOOT``, ``NORM`` -- with a tooltip explaining
  each, so what a saved intensity has had applied to it is visible without
  opening anything. The mask tool and the reciprocal-space reconstruction
  settings are reachable from inside the dialog, and the mask, solid-angle,
  polarization and normalization settings remain shared between the ROI
  integration and the reconstruction rather than duplicated. The beam profile
  is now configured once for the whole session instead of separately for
  rocking and stationary integration.
- The footprint correction options gained a preview of the beam profile,
  showing the normalized profile against position relative to the sample
  center, with the centroid of the beam marked, for measured and analytical
  profiles alike. The sample centering and offset now apply to analytical
  shapes as well, so an alignment error can be modelled without a measured
  profile.
...
- Versions up to and including v1.5.0 under-subtracted background when more
  than one background ROI was used, including the default ``Add All``
  configuration; they could therefore overstate saved ``croibg`` and
  ``F2_hkl`` intensities and report incorrect errors. The correction also
  aligns integration endpoint handling and propagated uncertainties.
  Regenerate affected measurements by repeating the final rocking integration
  with this version. The saved source rocking ROI curves and ROI definitions
  are sufficient when they remain in the Nexus database; re-integrating the
  original detector images is not required.
...
- Added a centralized, out-of-core reciprocal-space reconstruction pipeline.
  ...
  runs, and resumes jobs locally or from the command line via a new
...
- Replaced the Parquet-based scratch format and its separate reduce phase
  with checkpointed HDF5 scratch files: scratch/checkpoint file count is now
  driven by data volume versus the configured memory budget (a small,
  user-configurable ``Checkpoint count`` floor, default 10), not by output
  grid chunking. This fixes a real crash (``OSError: Too many open files``)
