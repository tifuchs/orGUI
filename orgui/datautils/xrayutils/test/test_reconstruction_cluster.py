...
def test_cluster_scripts_create_map_array_and_dependent_finalizer(
    ...
    """Generated scripts allocate separate map and finalize resources, and
    bake --total-tasks into both commands rather than relying on any
    scheduler environment variable for the array size (design doc
    Sec13: most schedulers besides Slurm never expose a reliable
    total-array-size variable to a running task)."""
    ...
    assert "--total-tasks 17" in map_text
    assert "--total-tasks 17" in finalize_text
