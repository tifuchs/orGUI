...
``search image``
   Displays the image assigned to the active reference reflection and centers
   the detector view on the reflection marker.

``select Image``
   Assigns the currently displayed scan image to the active reference
   reflection. The image number is part of the measured diffractometer angle
   set, so this step changes the measured momentum-transfer vector.

``2D peak search``
   Runs a local center-of-mass peak search around the current marker. The
   dialog first asks for a coarse detector ROI and scan-axis range, then uses
   the refined result as the start point for a finer search. If a detector mask
   is set, masked pixels are excluded.
